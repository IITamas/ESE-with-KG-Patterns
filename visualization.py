...
def remove_fluff(s):
    return ", ".join(
        [
            a.split(":")[-1] if "http" not in a else a.split("/")[-1].split(":")[-1]
            for a in s.split(", ")
        ]
    )
...
def multi_graph_viz_from_path(paths):
    ...
    for index, path in enumerate(paths[0]):
        for index2, (entity1, edge, entity2) in enumerate(path):
            if (
                get_sorted_list_in_str(entity1) + "-" + get_sorted_list_in_str(entity2)
                not in g
                or edge
                not in g[
                    get_sorted_list_in_str(entity1)
                    + "-"
            ...
            if index2 == len(path) - 1:
                ...
                if (
                    len(path) == 1
                    and get_sorted_list_in_str(entity1)
                    == get_sorted_list_in_str(paths[3])
                ) or (
                    len(path) > 1
                    and get_sorted_list_in_str(entity1)
                    == get_sorted_list_in_str(paths[2])
                ):
                    color2 = "green"
